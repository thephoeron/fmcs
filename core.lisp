...
(defun STANDARD-METHOD-COMBINATION (self class-env inst-env selector applicable-methods args)
...
(defun SIMPLE-METHOD-COMBINATION (self class-env inst-env selector applicable-methods args)
  (declare (ignore selector))
...
(defun SAVE-COMBINED-METHOD (class-env selector method-combination-fn applicable-methods)
...
(defun REMOVE-INVALID-COMBINED-METHODS (a_class selector)
  (let ((method-entry (gethash selector (mcs-slot-value a_class (index-of-methods)))))
...
(defmacro DEFMETHOD ((a_class . qualifier-and-selector) parameter-list &rest body)
...
(defmacro DEFCLASS (a_class a_list-of-instance-variables a_list-of-superclasses &key (metaclass 'standard-class))
  `(setq ,a_class
...
(defmacro DEFMETACLASS (a_class a_list-of-instance-variables a_list-of-superclasses &key (metaclass 'standard-class))
  `(setq ,a_class
         (funcall (mcs-slot-value ,metaclass (index-of-basicnew-fn))
                  ,metaclass
                  :name ',a_class
